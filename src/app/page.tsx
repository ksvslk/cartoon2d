...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Screenplay parser: deterministically extract dialogue lines and inject into beats
      if (isLongScript) {
        setStoryData(prev => {
          if (!prev) return prev;
          const newBeatCount = prev.beats.length - initialBeatsLength;
          ...
          // Parse "CharacterName: dialogue text" lines from the prompt
          ...
          if (dialogueLines.length === 0) return prev;

          console.log(`[Screenplay Parser] Found ${dialogueLines.length} dialogue lines from ${new Set(dialogueLines.map(d => d.speaker)).size} characters`);

          // Build actor ID lookup: "Plesiosaurus" → "actor-plesiosaurus"
          const actorLookup = new Map<string, string>();
          prev.actors_detected.forEach(a => {
            actorLookup.set(a.name.toLowerCase(), a.id);
            // Also try matching the ID stem
            const stem = a.id.replace('actor-', '').replace(/-/g, ' ');
            actorLookup.set(stem, a.id);
          });

          const resolveActorId = (speaker: string): string => {
            const lower = speaker.toLowerCase();
            // Direct name match
            if (actorLookup.has(lower)) return actorLookup.get(lower)!;
            // Partial match
            for (const [name, id] of actorLookup) {
              if (lower.includes(name) || name.includes(lower)) return id;
            }
            // Fallback: create ID from name
            return `actor-${lower.replace(/\s+/g, '-')}`;
          };

          // Distribute dialogue lines evenly across beats
          const newBeats = [...prev.beats];
          const linesPerBeat = Math.ceil(dialogueLines.length / newBeatCount);

          for (let beatIdx = 0; beatIdx < newBeatCount; beatIdx++) {
            const globalIdx = initialBeatsLength + beatIdx;
            const startLine = beatIdx * linesPerBeat;
            const endLine = Math.min(startLine + linesPerBeat, dialogueLines.length);
            const beatDialogueLines = dialogueLines.slice(startLine, endLine);

            if (beatDialogueLines.length === 0) continue;

            // Keep non-dialogue audio (sfx, music), replace dialogue entries
            const existingNonDialogue = (newBeats[globalIdx].audio || []).filter(a => a.type !== 'dialogue');
            const newDialogueEntries = beatDialogueLines.map((dl, i) => {
              // Estimate sequential timing: ~0.07s per character + 0.3s gap
              const prevDuration = i > 0
                ? beatDialogueLines.slice(0, i).reduce((sum, prev) => sum + (prev.text.length * 0.07) + 0.3, 0)
                : 0;
              return {
                type: 'dialogue' as const,
                actor_id: resolveActorId(dl.speaker),
                text: dl.text,
                delivery_style: 'neutral',
                start_time: prevDuration,
              };
            });

            newBeats[globalIdx] = {
              ...newBeats[globalIdx],
              audio: [...existingNonDialogue, ...newDialogueEntries],
            };
          }
