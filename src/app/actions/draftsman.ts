...
export async function processDraftsmanPrompt(base64Image: string, entityDescription: string, requiredViews: string[] = ['view_front']): Promise<DraftsmanResponse> {
    const viewsList = requiredViews.map(v => `- \`<g id="${v}">\``).join('\n   ');
...
3. **Target Views (CRITICAL)**: Do NOT rig the character in the specific dynamic pose from the reference image. You MUST draw the character in generic, neutral standing poses (A-Pose or T-Pose) across EXACTLY the following top-level angle containers:
   ${viewsList}
   By default, make the first requested view visible, and any others \`display="none"\`.
...
7. **Visemes and Emotions (CRITICAL)**: Inside the 'head' group of ANY view container, you MUST generate two sub-containers: \`<g id="mouth_visemes">\` and \`<g id="emotions">\`.
...
8. **The JSON Rig**: You must define the explicit (x, y) absolute coordinates of the pivot point for *every single animatable bone* you created across ALL views. To avoid naming collisions, ensure bones within views are prefixed uniquely based on the view (e.g., \`front_arm_left\`, \`side_arm_left\`).
